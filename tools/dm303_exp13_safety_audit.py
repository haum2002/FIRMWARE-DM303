#!/usr/bin/env python3
"""Safety audit for the DM303 V4.0.1b exp13 firmware image.

This is read-only. It compares the final flashable image against the official
V4.0 reference and proves that every changed byte falls inside a known exp13
...
DEFAULT_REPORT = Path("docs/v401b-exp13-flash-safety-audit.md")

EXPECTED_CURRENT_SHA256 = "fbe05118e33743fb56679d5edd1eb2019c1d82e39f499c1459fe89401ce8130b"
...
def write_report(path: Path, official_path: Path, current_path: Path) -> bool:
    ...
    lines = [
        ...
        f"| Current hash matches exp13 final | `{bool_text(current_hash_ok)}` | `{current_hash}` |",
        ...
        f"| Critical exp13 patch bytes present | `{bool_text(critical_ok)}` | `{len(criticals)}` byte guards checked |",
...
def main() -> int:
    ...
    print(f"exp13_safety_audit_ok={ok}")
