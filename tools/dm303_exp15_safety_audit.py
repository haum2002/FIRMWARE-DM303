#!/usr/bin/env python3
"""Safety audit for the DM303 V4.0.1b exp15 firmware image.

The audit is read-only. It proves that every byte changed from official V4.0
is inside a known patch interval, and that the new exp15 latency bytes are
...
DEFAULT_REPORT = Path("docs/v401b-exp15-flash-safety-audit.md")

EXPECTED_OFFICIAL_SHA256 = "64faaffb5fb65bdd0057d4fce1d9a2ac93e9229f118fba0a84d758c0ff926158"
EXPECTED_CURRENT_SHA256 = "3a2db571a4c783d0df2a454ec13d8d38a3a22a0e6ad7cc9993a0afa1edd7f3a0"
...
ALLOWED_INTERVALS = [
    ...
    (0x09818, 0x0981A, "exp15 first stale bit0 early-error gate bypass"),
    (0x098B4, 0x098B6, "exp15 second stale bit0 early-error gate bypass"),
    (0x09950, 0x09952, "exp15 parsed stale bit0 early-error gate bypass"),
    ...
    (0x15812, 0x15814, "exp15 first immediate mode/range gate"),
    (0x15838, 0x1583A, "exp15 second immediate mode/range gate"),
    (0x1585E, 0x15862, "exp14 first current-switch latency cap"),
    (0x15862, 0x15864, "exp15 first return-to-mode gate"),
    (0x15888, 0x1588C, "exp14 second current-switch latency cap"),
    (0x1588C, 0x1588E, "exp15 second return-to-mode gate"),
...
CRITICAL_BYTES = {
    0x02CA0: ("visible main version marker", b"MT100MM V4.0.1c\x00"),
    0x02CB0: ("visible secondary version marker", b"BT100MM V4.0.1c\x00"),
    ...
    0x09818: ("exp15 stale bit0 gate A", bytes.fromhex("01 e0")),
    0x098B4: ("exp15 stale bit0 gate B", bytes.fromhex("01 e0")),
    0x09950: ("exp15 stale bit0 gate C", bytes.fromhex("03 e0")),
    0x0F19A: ("mode/range entry branch to wrapper", bytes.fromhex("1e f0 34 ba")),
    0x15812: ("exp15 immediate switch gate A", bytes.fromhex("00 bf")),
    0x15838: ("exp15 immediate switch gate B", bytes.fromhex("00 bf")),
    0x1585E: ("exp14 current-switch latency cap A", bytes.fromhex("b0 f5 c8 6f")),
    0x15862: ("exp15 immediate switch gate C", bytes.fromhex("00 bf")),
    0x15888: ("exp14 current-switch latency cap B", bytes.fromhex("b0 f5 c8 6f")),
    0x1588C: ("exp15 immediate switch gate D", bytes.fromhex("00 bf")),
...
def write_report(official_path: Path, current_path: Path, report_path: Path) -> bool:
    ...
    lines = [
        ...
        f"- All changed bytes inside allowed exp15 intervals: `{bool_text(diff_ok)}`",
        f"- Critical exp15 byte guards present: `{bool_text(critical_ok)}`",
...
def main() -> int:
    ...
    print(f"exp15_safety_audit_ok={ok}")
