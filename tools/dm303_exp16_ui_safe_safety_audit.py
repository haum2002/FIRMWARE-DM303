#!/usr/bin/env python3
"""Safety audit for the DM303 V4.0.1b exp16 UI-safe firmware image.

The audit is read-only. It proves that every byte changed from official V4.0
is inside a known patch interval, and that the exp16 UI-safe bytes are
...
DEFAULT_REPORT = Path("docs/v401b-exp16-ui-safe-flash-safety-audit.md")

EXPECTED_OFFICIAL_SHA256 = "64faaffb5fb65bdd0057d4fce1d9a2ac93e9229f118fba0a84d758c0ff926158"
EXPECTED_CURRENT_SHA256 = "29a8bd71cba65538ba25d8e76aa5122234f9a28ad957287ba82fbf4fdf6c60af"
...
CRITICAL_BYTES = {
    0x02CA0: ("visible main version marker", b"MT100MM V4.0.1d\x00"),
    0x02CB0: ("visible secondary version marker", b"BT100MM V4.0.1d\x00"),
...
def write_report(official_path: Path, current_path: Path, report_path: Path) -> bool:
    ...
    lines = [
        ...
        f"- All changed bytes inside allowed exp16 UI-safe intervals: `{bool_text(diff_ok)}`",
        f"- Critical exp16 UI-safe byte guards present: `{bool_text(critical_ok)}`",
...
def main() -> int:
    ...
    print(f"exp16_ui_safe_safety_audit_ok={ok}")
