...
Profile: `stream-recovery-exp15` - exp14 plus visible V4.0.1c marker, immediate mode-call gates, and stale bit0 error-gate bypass for the no-improvement field result.
...
- Two long meter/current transition guards that compared against `0x3e80` are capped to `0x0640`. This targets the reported approximately 30-second AC->DC ammeter blank while keeping a bounded settle guard in place.
...
- Three stream transfer helpers bypass stale bit `0` early error returns and continue into the normal helper body. This is an aggressive recovery test for the no-improvement exp14 result.
- Version strings are marked `V4.0.1c` as a visible exp15 flash marker.
...
- Output SHA-256: `3a2db571a4c783d0df2a454ec13d8d38a3a22a0e6ad7cc9993a0afa1edd7f3a0`
...
| `0x02ca0` | 16 | `4d 54 31 30 30 4d 4d 20 56 34 2e 30 20 00 00 00` | `4d 54 31 30 30 4d 4d 20 56 34 2e 30 2e 31 63 00` | preserve model ID and mark candidate version as V4.0.1 beta |
| `0x02cb0` | 16 | `42 54 31 30 30 4d 4d 20 56 34 2e 30 20 00 00 00` | `42 54 31 30 30 4d 4d 20 56 34 2e 30 2e 31 63 00` | preserve model ID and mark candidate version as V4.0.1 beta |
...
| `0x1585e` | 4 | `b0 f5 7a 5f` | `b0 f5 c8 6f` | stream-recovery-exp14: cap long current/meter switch gate from 0x3e80 to 0x0640 to reduce AC-to-DC recovery latency |
| `0x15888` | 4 | `b0 f5 7a 5f` | `b0 f5 c8 6f` | stream-recovery-exp14: cap second long current/meter switch gate from 0x3e80 to 0x0640 to reduce repeated blanking after mode return |
...
| `0x09818` | 2 | `08 b1` | `01 e0` | stream-recovery-exp15: bypass stale bit0 error gate in first stream transfer helper |
| `0x098b4` | 2 | `08 b1` | `01 e0` | stream-recovery-exp15: bypass stale bit0 error gate in second stream transfer helper |
| `0x09950` | 2 | `18 b1` | `03 e0` | stream-recovery-exp15: bypass stale bit0 error gate in parsed stream transfer helper |
| `0x045b2` | 4 | `01 f0 9c fc` | `29 f0 05 f8` | stream-recovery-exp15: route LOGO-1 resource load through boot stabilization delay wrapper |
| `0x2d5c0` | 14 | `00 00 00 00 00 00 00 00 00 00 00 00 00 00` | `10 b5 d8 f7 94 fc c8 20 da f7 7b fa 10 bd` | stream-recovery-exp15: call original LOGO-1 loader, wait 200 ticks, then return to boot loader |
