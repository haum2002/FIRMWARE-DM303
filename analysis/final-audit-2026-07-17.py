#!/usr/bin/env python3
"""Independent final audit of the DM303 V4.0.1o / V4.0.1p packages.

Fresh byte-level analysis written 2026-07-17. Deliberately does NOT import the
project checker modules: every expectation below is re-derived from the
official V4.0 backup and from first-principles Thumb-2 decoding, so the audit
fails loudly if any earlier assumption was wrong. Read-only against backup/.

Run from repo root with any stdlib Python 3:
    python analysis/final-audit-2026-07-17.py
"""
...
def main() -> int:
    ...
    check(len(official) == len(repair) == len(uims) == 203260, "firmware sizes 203260")
    ...
    for off in (0x09CA0, 0x0C6C8, 0x2C4EA):
        check(official[off:off + 2] == bytes.fromhex("fe e7")
              and repair[off:off + 2] == bytes.fromhex("fe e7")
              and uims[off:off + 2] == bytes.fromhex("fe e7"),
    ...
    print("== J. repair-i sd-root resources ==")
    ...
    print("== K. ui-ms sd-root resources ==")
    audit_sd_root(UIM_ROOT, expect_overlay=True, overlay_names=overlay_names)

    print("== L. TEXT_SP.DAT structure (ui-ms) ==")
    audit_text_dat(OFFICIAL_SYS / "TEXT_SP.DAT", UIM_ROOT / "system/TEXT_SP.DAT")

    print("== M. overlay BMP layout (ui-ms) ==")
    for name in sorted(n for n in overlay_names if n.endswith(".bmp")):
        audit_bmp(UIM_ROOT / "system" / name)

    print("== N. CHECKSUMS-SHA256.txt matches disk ==")
    bad = 0
...
def audit_text_dat(official_path: Path, cand_path: Path) -> None:
    off_entries = parse_text_dat(official_path.read_bytes())
    cand_entries = parse_text_dat(cand_path.read_bytes())
    check(len(off_entries) == len(cand_entries) == 773, "entry count 773 both files",
...
def audit_bmp(path: Path) -> None:
    ...
    check(ok, f"BMP RGB565 layout {path.name}")
